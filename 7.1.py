...
class GNet(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.ConvTranspose2d(2 * C, C, 3, 1, 1),
            nn.ReLU(),
            nn.Upsample(scale_factor=2),

            nn.ConvTranspose2d(C, C, 3, 1, 1),
            nn.ReLU(),
            nn.Upsample(scale_factor=2),

            nn.ConvTranspose2d(C, 1, 3, 1, 1),
        )
...
G = GNet().to(device)
D = DNet().to(device)
GO = torch.optim.Adam(G.parameters(), lr=LR, betas=(0.5, 0.99))
DO = torch.optim.Adam(D.parameters(), lr=LR, betas=(0.5, 0.99))
...
while True:
    ...
    d_loss = 0.5 * (lossf(D(real), vf) + lossf(D(fake.detach()), ff))
    d_loss.backward()
